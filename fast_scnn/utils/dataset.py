...
class CityScapesDataset(object):
    ...
    def preprocess(self, img, label):
        ...
        if self.input_size_factor is not None:
            img_size = img.shape
            new_img_size = img_size * self.input_size_factor
            img = tf.image.resize(img, new_img_size.astype(int), method=tf.image.ResizeMethod.BILINEAR, preserve_aspect_ratio=True)
            label = tf.image.resize(label, new_img_size.astype(int), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR, preserve_aspect_ratio=True)
